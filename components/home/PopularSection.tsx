      ...
      <div className="grid grid-cols-3 gap-3 pb-[60px]">
        {isLoading ? (
          Array.from({ length: 12 }).map((_, index) => (
            <Skeleton key={index} className="w-full h-[160px] rounded-md border-none" />
          ))
        ) : (
          data && Array.isArray(data) ? data.map((anime, index) => (
            <Link href={`/${encodeURI(anime.title)}`} key={index} className="flex flex-col gap-2">
              <div className="relative w-full h-[160px]">
                {imageLoading[index] && <Skeleton className="absolute inset-0 rounded-md" />}
                <img 
                  src={anime.poster || ""} 
                  alt={anime.title} 
